...
class PaymentMethodController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($type == 'currency_manage') {
            $exrate = PaymentMethod::refreshCache($request->input('base_currency'));
            ...
            Setting::updateValue('pmc_fx_exrates', json_encode($exrate));
            ...
            foreach (PaymentMethod::Currency as $code => $val) {
                ...
                $own_rate = $request->input('pmc_rate_' . $currency) == null ? 1 : $request->input('pmc_rate_' . $currency);
                $all_new_rate[$currency] = $own_rate;
                Setting::updateValue('pmc_rate_' . $currency, $own_rate);
            }
            // v1.1.1
